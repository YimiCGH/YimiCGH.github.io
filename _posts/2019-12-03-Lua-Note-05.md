...
|函数|参数|返回|说明|
|-|-|-|-|
|coroutine.create(f)||返回一个协程的控制器|创建一个新的协程|
...
|coroutine.wrap(f)||返回一个协程的调用函数，相当于一个委托函数|wrap()也是用来创建协程的。不同的是，它不返回协程本身，而是返回一个函数，每次调用这个函数相当于执行resume()函数，调用这个函数是传入的参数相当于调用resume是传入的变长参数。和resume不同，它不是在保护模式下执行的，执行崩溃时会直接向外抛出|
