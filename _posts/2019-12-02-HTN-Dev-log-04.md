...
从结果来看，先不看结果是否正确，可以发现TFD算法存在一些缺陷。就和寻路算法一样，简单的前向搜索可以保证得到解决方案，但不保证是最优，而且，往往是比较长（差）的结果。针对本次测试，得到的解决方案一共有33步，而这个问题的最短路径（最优解决方案）是12步，因此，如果这个解决方案没错的话，很明显还需要优化，也就是使用类似A*之类，局部搜索，后向搜索等方法进行优化。
